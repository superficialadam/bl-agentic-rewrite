...
#
# Usage:
#   ./loop.sh plan [max]       # Plan mode
#   ./loop.sh build [max]      # Build mode
...
while true; do
  ...
  LOGFILE="ralph/logs/loop-${MODE}-${ITERATION}-${TIMESTAMP}.jsonl"
  ...
  if [ -f "$STOP_FILE" ]; then
    echo ""
    echo "  🏁 Stop signal received — complete"
    rm -f "$STOP_FILE"
    break
  fi
